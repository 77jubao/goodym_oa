...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/oa/oaOvertime/">加班记录列表</a></li>
		<li class="active"><a href="${ctx}/oa/oaOvertime/form?id=${oaOvertime.id}">加班记录<shiro:hasPermission name="oa:oaOvertime:edit">${not empty oaOvertime.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="oa:oaOvertime:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="oaOvertime" action="${ctx}/oa/oaOvertime/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:select path="overtimeType" class="input-xlarge ">
		...
		<div class="control-group">
			<label class="control-label">加班日期：</label>
			<div class="controls">
				<input name="overtimeDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate "
					value="<fmt:formatDate value="${oaOvertime.overtimeDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
		...
		<div class="control-group">
			<label class="control-label">转换小时数：</label>
			<div class="controls">
				<form:input path="changeHour" htmlEscape="false" maxlength="255" class="input-xlarge "/>
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
				<input name="expiredDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate "
					value="<fmt:formatDate value="${oaOvertime.expiredDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
				<form:select path="overtimeStatus" class="input-xlarge ">
					<form:option value="" label=""/>
					<form:options items="${fns:getDictList('')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
				</form:select>
			</div>
